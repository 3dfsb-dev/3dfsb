...
if uname -s | grep -i -c "LINUX" > /dev/null; then 
	...
	#options="/usr/lib/x86_64-linux-gnu/libX11.so /usr/lib/x86_64-linux-gnu/libSDL.so $prefix/libgstreamer-0.10.so $prefix/libgobject-2.0.so $prefix/libgstinterfaces-0.10.so.0.25.0"
	options="/usr/lib/x86_64-linux-gnu/libX11.so -Wl,$prefix/libsmpeg-0.4.so.0.1.4"
    gcc -v $ARGS1 $ARGS2 -I/usr/local/include -I/usr/include/ -I/usr/X11R6/include -L/usr/lib/ -L/usr/local/lib/ -L/usr/X11R6/lib -D_THREAD_SAFE -O2 -g -x c $(pkg-config --libs --cflags gstreamer-0.10) -lSDL_image -lGL -lGLU -lglut -lXmu -lXi -lXext -lX11 -lm -lsmpeg -o tdfsb \
-Wl,$options \
tdfsb.c;
